...
| 2026-04-18 | Claude Code 스타일 자연어 REPL 추가. `langchain-harness`(인자 없음) 실행 시 프롬프트가 뜨고, 사용자가 `user` pseudo-member로 팀에 등록되어 lead DeepAgent와 mailbox 기반 대화. 슬래시 커맨드(`/help /team /status /teams /new /resume /spawn /inbox /task /memory /clear /exit`). rich 렌더러 + prompt_toolkit 히스토리. `langchain-harness team ...` 기존 CLI는 유지. 의존성 `rich`, `prompt_toolkit` 추가. 패키지 버전 0.2.0 → 0.4.0 | src/langchain_harness/repl/{__init__.py,app.py,session.py,lead_factory.py,commands.py,renderer.py}, src/langchain_harness/cli.py, src/langchain_harness/__init__.py, pyproject.toml | 사용자 요청 — langchain_harness를 Claude Code처럼 자연어로 대화하며 팀을 조율할 수 있게 |
